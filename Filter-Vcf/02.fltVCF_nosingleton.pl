...
#beta
my %opts = (m=>10,M=>60,S=>0,s=>100000,l=>'',r=>0);
getopts('BNDm:M:QS:Al:r:s:',\%opts);
die("
...
Options: -B        remove non biallele sites
	 ...
	 -A        filter singleton alle  (need -N)

Notice : 1.in.vcf should be a joint calling vcf made by sentieon.
	 2.If you need choose SNP by interval make sure you use -l -s -r ,don't miss any of them.

Guide : step1 : whole genome high quality non-missing SNP
		fltVCF_xll.pl -B -Q -N <in.vcf> <out.vcf> 2>prefix.log
	step2 : filter singleton
		fltVCF_xll.pl -A <in.vcf> <out.vcf> 2>prefix.log
        step3 : independent SNP 
                fltVCF_xll.pl -S -s -l -r <in.vcf> <out.vcf> 2>prefix.log
	notice: run it step by step will be more helpful! Good luck ~.~
		filter depth before you do these steps use fltDEP_xll_gz.pl
...
while (my $line =<IN>){
	...
	if (defined($opts{A}) && ($het == 1 || $dlens - $het == 1)){
		$maf = 1;$count_A++;
	}
